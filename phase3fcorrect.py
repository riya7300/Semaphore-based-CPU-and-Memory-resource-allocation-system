...
# ====================================================================
# EMBEDDED C CODE (Priority + Banker's + Paging) - UNCHANGED
# NOTE: The C code's scheduler() logic in the original file uses a 
# combined approach where it *always* selects based on Priority, then 
# FCFS (tie-breaker), but FCFS/RR are handled by setting the burst time.
# IMPORTANT: To make FCFS/RR truly *ignore* priority in the scheduler(),
# the C code's scheduler must be refactored. The fix below addresses 
# the UI request *assuming* the scheduler() logic will be corrected 
# to select based on ARRIVAL TIME for FCFS/RR, but for now, we just 
# hide the column as requested.
# I will apply the required C-code scheduler logic fix to correctly 
# implement pure FCFS/RR selection before providing the final code.
# ====================================================================
...
// Dynamic Settings
char ALGORITHM[20]; // Increased size to accommodate "Priority-FCFS"
...
// ====================================================================
// SCHEDULER (Priority Preemptive + RR/FCFS) - CORRECTED FOR PURE FCFS/RR
// ====================================================================

void scheduler() {
    int completed = 0;
    int is_rr = (strcmp(ALGORITHM, "RR") == 0);
    int is_prio_fcfc = (strcmp(ALGORITHM, "Priority-FCFS") == 0); 
    ...
    while (completed < NUM_PROCESSES) {
        ...
        // A. Priority-based selection (Only for Priority-FCFS)
        if (is_prio_fcfc) {
            for (int i = 0; i < NUM_PROCESSES; i++) {
                if (processes[i].state != FINISHED && processes[i].remaining_time > 0) {
        ...
        // B. FCFS-based selection (For pure FCFS and RR)
        else { 
            for (int i = 0; i < NUM_PROCESSES; i++) {
                if (processes[i].state != FINISHED && processes[i].remaining_time > 0) {
                    // FCFS/RR: Select the process with the earliest arrival time (ignoring priority)
                    if (processes[i].arrival_time < earliest_arrival) {
                        earliest_arrival = processes[i].arrival_time;
                        next_pid_to_run = i;
                    } 
                    // Tie-breaker: If arrival times are equal, fall back to PID (FCFS)
                    else if (processes[i].arrival_time == earliest_arrival && next_pid_to_run != -1 && processes[i].id < processes[next_pid_to_run].id) {
                         next_pid_to_run = i;
                    }
                }
            }
        ...
        if (processes[i].mem_allocated == processes[i].mem_needed && acquire_cpu(i)) {
            ...
            int execTime;
            if (is_rr) {
                 // RR: Run for Quantum or Remaining Time
                 execTime = (processes[i].remaining_time > quantum_s) ? quantum_s : processes[i].remaining_time;
            } else if (is_prio_fcfc || strcmp(ALGORITHM, "FCFS") == 0) { 
                 // FCFS or Priority-FCFS: Run to completion (non-preemptive burst time)
                 execTime = processes[i].remaining_time; 
            } else {
                 // Fallback/Default step time (e.g., 1 second)
...
# ====================================================================
# PYTHON GUI CODE (Tkinter) - MODIFIED FOR DYNAMIC PRIO COLUMN
# ====================================================================
...
class SemaphoreUI(tk.Tk):
    ...
    def create_widgets(self):
        ...
        ttk.Combobox(settings_frame, textvariable=self.algo_var, values=["RR", "FCFS", "Priority-FCFS"], width=13, state="readonly").grid(row=0, column=3, padx=(0, 10))
        # ----------------------------------------------------------------------------------
        ...
        # Initial Treeview setup (will be configured in update_ui_with_snapshot)
        initial_columns = ("PID", "Prio", "Burst (s)", "Remaining (s)", "Mem Req", "Max Mem (Banker's)", "Frames Alloc", "Status")
        self.tree = ttk.Treeview(card_proc, columns=initial_columns, show="headings", height=10)
        
        # Initial column configuration (for default Priority-FCFS display or initial setup)
    ...
    def start_simulation(self):
        ...
        try:
            ...
            if self.snapshot_count > 0:
                ...
                # Display the first snapshot and set up the UI dynamically
    ...
    def update_ui_with_snapshot(self, snapshot):
        ...
        # Determine columns based on algorithm
        algo = self.algo_var.get()
        show_prio = (algo == "Priority-FCFS")
        
        # 1. Define Columns and Widths
        if show_prio:
            columns = ("PID", "Prio", "Burst (s)", "Remaining (s)", "Mem Req", "Max Mem (Banker's)", "Frames Alloc", "Status")
            col_widths = {"PID": 50, "Prio": 50, "Burst (s)": 80, "Remaining (s)": 100, "Mem Req": 80, "Max Mem (Banker's)": 130, "Frames Alloc": 100, "Status": 180}
        else:
            columns = ("PID", "Burst (s)", "Remaining (s)", "Mem Req", "Max Mem (Banker's)", "Frames Alloc", "Status")
            col_widths = {"PID": 50, "Burst (s)": 80, "Remaining (s)": 100, "Mem Req": 80, "Max Mem (Banker's)": 130, "Frames Alloc": 100, "Status": 200}
        
        # 2. Reconfigure Treeview
        # Remove all existing columns first (needed for dynamic changes)
        self.tree.config(columns=columns)
        
        # Clear existing headings and redefine them for the new columns
        for col in self.tree.get_children():
            self.tree.delete(col)
        
        # Set new headings and column sizes
        for col in columns:
            self.tree.heading(col, text=col)
            self.tree.column(col, anchor="center", width=col_widths.get(col, 80))
        ...
        for i, proc in enumerate(processes):
            ...
            # Dynamically set values tuple
            if show_prio:
                values = (pid, prio, burst, remaining, mem_needed, max_mem, mem_allocated, status)
            else:
                values = (pid, burst, remaining, mem_needed, max_mem, mem_allocated, status)

            self.tree.insert("", tk.END, values=values, tags=(tag,))
        
        # --- Update Contention Metrics Labels ---
    ...
    def show_final_report(self):
        try:
            ...
            # 2. Formatted Report
            header = f"--- SCHEDULER: {algo} ({data.get('numProcesses', 0)} Processes) ---\n"
            ...
            if show_prio:
                 ...
                 header += f"{'PID':<4}{'Burst':>8}{'MemReq':>8}{'MaxMem':>8}{'Completion':>12}{'Turnaround':>12}{'Waiting':>10}\n"
                 separator_length = 68 
                 
            self.report_text.insert(tk.END, header)
            self.report_text.insert(tk.END, "=" * separator_length + "\n")
            ...
            for p in procs:
                if show_prio:
                    ...
                    line = f"P{p['id']:<3}{p['burst']:>8}{p['memNeeded']:>8}{p['maxMem']:>8}{p['completion']:>12}{p['turnaroundTime']:>12}{p['waitingTime']:>10}\n"
